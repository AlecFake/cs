...
对于读未提交来说，直接返回的的数据的最新值。对于读提交和可重复读来说，数据库里会创建一个视图，访问的时候以视图的逻辑结果为准。对于读提交而言，这个视图是在每个SQL语句开始执行的时候创建的，而对可重复读而言，视图则是则是在启动时创建的，整个事务存在期间都在用这个视图。对于串行化来说，直接使用加锁的方式来避免并行访问。
